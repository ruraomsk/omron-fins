 ...
 */
    static final int Crc(byte[] buffer, int pos, int len)
    {
        if((len&1)>0) {
            buffer[pos+len] = 0;
            len++;
        ...
        for (int i = 0; i < (len >> 1); i++)
        {
            ...
            crc = (crc + ToShort(buffer, pos)) & 0x7fff;
            pos += 2;
        }
        return crc & 0x7fff;
    }
 ...
 */
    ...
    public static boolean FP_DEBUG = false;
    ...
    static final byte FP_CODE_INFO=1;
    static final byte FP_CODE_34H=0x34;    //Передача настроечных данных по инициативе серверов уровня 2 или запрос о настроечных данных в контроллере
    static final byte FP_CODE_35H=0x35;     //Квитирование полученных настроечных данных
    static final byte FP_CODE_36H=0x36;    //Передача настроечных данных из контроллера на серверы уровня 2 по запросу поступившему от серверов уровня 2
    static final byte FP_CODE_30H=0x30;     //Диагностика
    static final byte FP_CODE_10H=0x10;     //Управление
    static final byte FP_CODE_91H=(byte)0x91;     //Синхронизация времени    
    static final byte FP_CODE_64H=0x64;    //Контрольное сообщение от серверов уровня 2    
