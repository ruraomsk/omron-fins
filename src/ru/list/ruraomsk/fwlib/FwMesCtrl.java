 ...
 */
class FwMesCtrl extends FwBaseMess
{
 ...
 * @param commandcode 
 */
    FwMesCtrl(int controller, int commandcode)
    {
    ...
    {
        if(idx>=params.length||idx<0) return;
        params[idx].par = value;
    }
    public int getPar(int idx)
    {
        if(idx>=params.length||idx<0) return 0;
    ...
    {
        int par = 0;
        byte good = FwUtil.FP_DATA_NOGOOD;
    }
